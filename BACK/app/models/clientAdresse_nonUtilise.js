...
class ClientAdresse {
    ...
    static async findByTitre(titre) {
        ...
        if (!rows[0]) {
            console.log("Aucune adresse avec ce titre n'éxiste déja.")
            return null;
        }
    ...
    async saveWithEnvoie() {
        ...
        const {
            ...
            `INSERT INTO mada.client_adresse (prenom, nom_famille, ligne1, ligne2, ligne3, telephone, envoie, titre, created_date, id_client, id_ville) VALUES ($1, $2, $3, $4, $5, $6, TRUE, $7, now(), $8, $9) RETURNING *;`,
            [this.prenom, this.nomFamille, this.ligne1, this.ligne2, this.ligne3, this.telephone, this.titre, this.idClient, this.idVille]
        );
        this.id = rows[0].id;
        this.createdDate = rows[0].created_date;
    ...
    async envoieTrue() {
        const {
            ...
            `UPDATE mada.client_adresse SET envoie = TRUE WHERE id = $1 RETURNING *;`,
            [this.id]
        );
        this.envoie = rows[0].envoie;
        console.log(
            `La valeur envoie du le ClientAdresse id : ${this.id} a été passé a ${this.envoie} avec succés !`
        );
    ...
    async envoieNull() {
        const {
            ...
            `UPDATE mada.client_adresse SET envoie = NULL WHERE id = $1 RETURNING *;`,
            [this.id]
        );
        this.envoie = rows[0].envoie;
        console.log(
